export default async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ 
      active, 
      sold, 
      stats: { toCollect, ifAllSold, listedValue },
      debug: {
        totalFromApi: data.auctions.length,
        withBids: data.auctions.filter(a => a.bids && a.bids.length > 0).length,
        withHighestBid: data.auctions.filter(a => a.highest_bid_amount > 0).length,
        endedPastNow: data.auctions.filter(a => a.end && a.end <= now).length,
        after2025: data.auctions.filter(a => a.end && a.end >= cutoff).length,
        soldDetected: data.auctions.filter(a => isSold(a)).length,
        sampleAuction: data.auctions.length > 0 ? {
          item: data.auctions[0].item_name,
          end: data.auctions[0].end,
          endDate: new Date(data.auctions[0].end).toISOString(),
          highest_bid: data.auctions[0].highest_bid_amount,
          starting_bid: data.auctions[0].starting_bid,
          claimed: data.auctions[0].claimed,
          bidsCount: (data.auctions[0].bids || []).length,
          bin: data.auctions[0].bin
        } : null
      }
    }), { status: 200, headers });
